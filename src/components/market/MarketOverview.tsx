...
export const MarketOverview = () => {
  ...
  const summarizeInsights = (text: string): string => {
    ...
    // 🔧 REMOVER BLOCOS JSON INTEIROS
    let cleanText = text.replace(/```json[\s\S]*?```/gi, '');
    cleanText = cleanText.replace(/```[\s\S]*?```/gi, '');
    cleanText = cleanText.replace(/\{[\s\S]*?"summary"[\s\S]*?\}/gi, '');
  ...
  const extractTop5Actions = (text: string): string[] => {
    ...
    // 🔧 REMOVER BLOCOS JSON ANTES DE PROCESSAR
    let cleanText = text.replace(/```json[\s\S]*?```/gi, '');
    cleanText = cleanText.replace(/```[\s\S]*?```/gi, '');
    cleanText = cleanText.replace(/\{[\s\S]*?"summary"[\s\S]*?\}/gi, '');
